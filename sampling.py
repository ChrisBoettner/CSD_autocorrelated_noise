...
def p_calculation(n):
    ...
    # variance
    ts_v = rolling_variance(ts_c, window_size); ts_vp = trend_test(ts_v[window_size:,:], memory_saving = memory_saving)
    # autocorrelation
    ts_a = rolling_autocorr(ts_c, window_size); ts_ap = trend_test(ts_a[window_size:,:], memory_saving = memory_saving)
    # phi fit
    ts_l = first_order_phi_OLS(ts_c, window_size); ts_lp = trend_test(ts_l[window_size:,:], memory_saving = memory_saving)
